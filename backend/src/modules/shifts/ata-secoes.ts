 ...
 *
 * Transcrição estruturada do LIVRO ATA — AI 03. Cada seção existe porque
 * aparece no livro de papel; nenhuma foi inventada aqui. Manter esta lista
...
export interface SecaoAta {
  chave: string;
  titulo: string;
  tipo: 'texto' | 'lista' | 'sim_nao_detalhe';
  ajuda: string;
...
export const SECOES_ATA: SecaoAta[] = [
  ...
  { chave: 'casa', titulo: 'Organização, manutenção e fatos gerais', tipo: 'texto', obrigatoria: false,
    ajuda: 'Estado da casa, manutenções pedidas ou realizadas, fatos gerais do turno.' },
