...
|---|---|
| `der.md` | as 115 tabelas com o que cada coluna guarda. É referência de dado, não narrativa, e o `documentacao.spec.ts` cobra que toda tabela apareça lá |
...
| **139 migrações** | `.sql` dentro das partições |
| **115 tabelas** | `CREATE TABLE` nas migrações |
| **90 suítes** | `backend/test/*.spec.ts` |
| **880 testes** | `it(` / `test(` nas suítes |
...
139 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              90 suítes (e2e contra PostgreSQL real + estáticas)
...
**O que sobra nesta seção, então, é nada** — e a §10.2 continua sendo o único item
aberto do §9, do lado da Fundação: quem lê a ATA Geral de dia, para os cargos além
de quem corrige.
...
Constrói, cria um banco virgem, aplica as 139 migrações **pelo binário
