...
| **136 migrações** | `.sql` dentro das partições |
| **115 tabelas** | `CREATE TABLE` nas migrações |
| **89 suítes** | `backend/test/*.spec.ts` |
| **871 testes** | `it(` / `test(` nas suítes |
...
136 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              89 suítes (e2e contra PostgreSQL real + estáticas)
...
Constrói, cria um banco virgem, aplica as 136 migrações **pelo binário
