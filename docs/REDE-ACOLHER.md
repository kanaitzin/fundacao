...
4. **`GET /timeline/all`** e **`GET /transfers/pending`** — duas leituras de
   lista. *Sondadas na fase 148: as duas respondem.* A segunda exige `houseId`, e
   sem ele devolve a frase crua do framework em inglês — o que vale para toda rota
   com `ParseUUIDPipe` em parâmetro de consulta, e é erro de programação, não de
   quem usa a tela. Fica anotado, não consertado.
