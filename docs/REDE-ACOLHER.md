...
| **140 migrações** | `.sql` dentro das partições |
| **116 tabelas** | `CREATE TABLE` nas migrações |
| **91 suítes** | `backend/test/*.spec.ts` |
| **892 testes** | `it(` / `test(` nas suítes |
...
140 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              91 suítes (e2e contra PostgreSQL real + estáticas)
...
Constrói, cria um banco virgem, aplica as 140 migrações **pelo binário
