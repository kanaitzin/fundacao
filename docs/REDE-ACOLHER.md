...
|---|---|
| `der.md` | as 117 tabelas com o que cada coluna guarda. É referência de dado, não narrativa, e o `documentacao.spec.ts` cobra que toda tabela apareça lá |
...
| **153 migrações** | `.sql` dentro das partições |
| **117 tabelas** | `CREATE TABLE` nas migrações |
| **106 suítes** | `backend/test/*.spec.ts` |
| **1007 testes** | `it(` / `test(` nas suítes |
...
| protótipo com **≈1344 KB** | `prototipo/rede-acolher-prototipo.html` |
...
limpo nos dois lados, **os sete ensaios de navegador limpos** (132 telas nos
...
| 142 | **A folha da guarita dizia QUEM podia entrar e não dizia QUANDO — e retirar alguém não deixava o motivo escrito.** Dois pedidos da Fundação em 22/09: *"todos devem ter horários e dias de visitação"*, a portaria *"vendo os dias de visita de cada um deles"*, e *"caso alguém seja removido que tenha motivos para tal escrito para registro"*. **O primeiro defeito é no portão, e não na tela:** sem o quando, a folha transfere para o porteiro uma decisão que é da casa — às 21h de uma terça ele tem duas saídas, barrar um familiar autorizado ou deixar entrar fora da hora, e nenhuma das duas é dele. **O segundo aparece seis meses depois:** a auditoria registrava QUEM retirou e QUANDO — e nada mais, porque log não copia conteúdo sensível (§5) e o motivo de uma retirada conta algo sobre uma família; então a família chega ao portão, ouve *"não está na folha"*, e não há ninguém na casa que saiba responder por quê. **O dia e a hora são DO VISITANTE, não da casa:** a avó que vem de ônibus de outra cidade vem no sábado de manhã, o padrinho que trabalha até as 18h vem à noite, e há visita que a Justiça marcou em dia fixo — uma regra única seria mais simples e estaria errada. **Opcionais no banco, cobrados na tela:** é a mesma decisão da foto 3×4 na 1120, e pelo mesmo motivo — travar a autorização em quem ainda não combinou deixaria o visitante de verdade do lado de fora; o banco garante a FORMA (dia entre domingo e sábado, faixa que começa antes de terminar) e a folha diz por extenso quando não há: *"sem dia combinado — confirme com a casa"*, que é instrução e não vazio, porque um traço na coluna faz o porteiro adivinhar. **O motivo da retirada virou TABELA, e não coluna** (`contact_visit_change`), por duas razões que são regra da casa: nada se sobrescreve — um contato pode ser retirado, reautorizado e retirado de novo, e uma coluna guardaria só a última, apagando justamente a que explica a história — e o motivo não é metadado, então não vai para o log; vive numa tabela append-only lida pelos três que respondem por quem entra. **O educador continua lendo a LISTA de contatos** (decisão de 03/09: ele precisa saber quem é a madrinha que aparece no portão) e **não lê o motivo**: espalhar um juízo sobre um familiar pelo plantão inteiro muda o que a técnica se sente à vontade para escrever, e frase que não se escreve não protege ninguém. *Dois erros meus, os dois pegos por teste.* **(1) A regra do horário era sobre o pedido, e tinha de ser sobre o RESULTADO:** eu exigia o horário no corpo da chamada, e então REAUTORIZAR alguém pedia à técnica que digitasse de novo o que já estava guardado — exatamente o caso da família cuja visita a Vara suspendeu e voltou a liberar. **(2) Usei duas classes de CSS que não existem** (`row wrap`, `f inline`) e um `fieldset` que não é o desenho desta casa; os dias da semana passaram a ser desenhados como a tela da Rotina já os desenha — `opts` com `aria-pressed` —, porque é a mesma pergunta feita em duas telas e a educadora que aprendeu numa não reaprende na outra. *E a semana mudou de casa:* `DIAS_DA_SEMANA` morava na partição da rotina, e a portaria passou a precisar dela — foi para o kernel, em arquivo PRÓPRIO e não dentro do `tempo.ts`, porque o protótipo importa esse vocabulário do backend de propósito (regra 14) e o `tempo.ts` lê `process.env`, que não existe no navegador. O que não podia divergir não é o nome do dia, é a NUMERAÇÃO: uma lista que contasse a semana a partir da segunda faria a folha impressa trocar terça por quarta. **E a suíte da fase 92 pegou duas coisas minhas, o que é o trabalho dela.** *(1)* A ORDEM DAS RECUSAS estava invertida: para um contato com aproximação restrita eu respondia *"falta combinar o horário"* — e quem lê isso vai combinar o horário e tentar de novo; a resposta certa é a restrição, que diz que isto se resolve com a equipe técnica antes. *(2)* Ela LIA A LINHA DA FOLHA POR POSIÇÃO (`madrinha[5]` era o CPF), e a coluna nova entrou antes dele: o teste passou a comparar o CPF com *"dom e sáb, das 09:00 às 11:30"* e reprovou uma folha certa. Índice fixo em tabela que cresce cobra o desenho de ontem; agora a linha se lê pelo conteúdo.
| 143 | **O educador não podia folhear os dias anteriores — e era uma lista de cargos, não uma regra.** A Fundação disse em 22/09, e deu a razão junto: *"as atas de dias passados podem ser vistas por todos, menos os registros e situações marcadas como confidencial; cada educador pode ver uma ata unificada da passagem dos dias anteriores para poder controlar e ajustar se necessário o comportamento ou a dinâmica da casa, assim como acompanhar a alimentação e o comportamental do todo dos atendidos."* **Medido, e é menos do que parecia: o DADO já era dele.** `ata_select` é `app_house_in_scope` — qualquer cargo da casa lê a ATA da casa, e sempre leu; `note_select` já exclui a linha **restrita** de quem não a alcança, para TODO cargo, o que quer dizer que a confidencialidade que ela descreveu mora no banco desde sempre. O que o barrava era UMA LISTA DE CARGOS na porta do arquivo, e a frase da tela dizia *"o arquivo das ATAS é da coordenação, da equipe técnica e dos líderes"*. Ele lia a ATA do turno ANTERIOR e mais nada. **E a janela de vários dias é outra pergunta, não um luxo:** o turno anterior responde *"o que houve ontem à noite"*; a dela é *"esta criança está comendo mal desde quando?"*, *"a casa está mais agitada esta semana?"* — e quem passa doze horas com a casa é quem primeiro percebe o padrão, e era quem não tinha como olhar para trás. **A ATA GERAL NOTURNA continua FORA, e isto é decisão de propósito.** `app_arquivo_atas` é `SECURITY DEFINER`: lá dentro o RLS não vale, e o `LEFT JOIN` com a `general_night_ata` devolveria o documento da noite a quem a `gna_select` não inclui — abrir o arquivo ao educador sem mexer nisso responderia, **de carona, à §10.2** (*"quem lê a ATA Geral de dia"*), que é pergunta ABERTA e é da Fundação. Então a Geral sai vazia para quem não a lê, e a lista de quem a lê ficou escrita em `app_le_ata_geral()` — uma função definer que ignora o RLS tem de repetir a regra que o RLS faria. **Duas frases que passaram a mentir, e as duas foram reescritas junto** — é a lição transversal deste repositório cobrando a si mesma. *(1)* A recusa do arquivo nomeava cargos, e a lista mudou duas vezes; agora ela diz *"é de quem trabalha nesta casa"* e manda falar com a coordenação se o vínculo não estiver lançado. *(2)* A aba da ATA Geral mandava *"use o Arquivo"* para todo cargo que não é o Líder Noturno Geral: antes da 1510 isso apontava para uma porta que o educador não tinha; depois dela, apontava para uma porta que existe e não responde — que é pior. **E o teste desta fase pegou um erro meu que é a regra 14 ao contrário:** eu corrigi a frase do arquivo no SERVIDOR DE MENTIRA e esqueci a do servidor, então o protótipo dizia a verdade e o produto não. *O que a tela faz agora é DIZER que a Geral não é dele e de quem é, em vez de só omitir: quem abre e não encontra, sem ler a frase, conclui que o sistema não a tem — e essa conclusão é a que chega à Fundação como "falta".*
| 144 | **As duas decisões que a conferência de 22/09 deixou para a Fundação, respondidas e viradas código.** *(1)* **Toda retirada da escala diz por quê.** Só o plantão JÁ PASSADO exigia o motivo, com o argumento da 1310 — *"mudar o futuro é organização; mudar o passado é dizer que a pessoa não estava lá"* —, que continua correto sobre o histórico e **não era o único em jogo**: quem pergunta *"por que eu fui tirado do plantão de sábado?"* pergunta na segunda, sobre um plantão que era futuro quando a mudança foi feita; a escala guardava quem tirou e quando, e ninguém para responder. Perguntada, a Fundação respondeu **sempre exigir**. *Vale para a SUBSTITUIÇÃO também, e isto é decisão da migração:* ela é uma retirada com uma entrada em cima, e *"a Joana entrou no lugar"* não explica por que a Marta saiu — é o que o `replaces_assignment_id` já diz sozinho; deixar só a substituição sem a frase abriria o caminho de sempre, quem não quisesse escrever substituiria em vez de retirar. **A frase da recusa muda com o tempo do plantão** porque o motivo é outro: no passado ela responde *"quem estava na casa naquela noite"*; no futuro, responde à pessoa que vai perguntar. E o `Substituído(a) por …` que o banco montava quando o motivo vinha vazio saiu: ele dizia o que o parentesco já diz, e nunca por quê. *(2)* **O Líder Diurno lê a ocorrência protegida.** A conferência achou uma assimetria e a levou a ela: o RELATO restrito o inclui desde a 0730, com a razão escrita — *"quem está com a criança às 23h precisa saber o que já foi registrado sobre ela, para não repetir uma pergunta que já feriu"* —, e o BLOCO PROTEGIDO da ocorrência (`iprot_select`, 0320) não. **Eu não desfiz a assimetria sozinho**, e a razão ficou escrita no §9: ocorrência protegida é material mais pesado que relato — fala espontânea e sinais observados, em suspeita de violência ou de abuso —, e ampliar quem a lê não é conserto de simetria. O educador continua fora; a SÍNTESE técnica também, e é outra coisa — ela é a leitura que a equipe técnica faz do caso, não o registro do que houve. **E a leitura do bloco protegido passou a deixar linha — para todos, não só para ele.** Não deixava para ninguém: a auditoria registrava quem ESCREVEU e não quem abriu, e ampliar o acesso ao material mais pesado do sistema sem rastro nenhum seria a única coisa desta fase que eu não saberia defender numa audiência. A casa já tinha o precedente — `person.judicial_view`, desde a fase 112. **Não é a porta com finalidade escrita**, que a Fundação recusou para este caso: nada é perguntado a quem abre o caso para trabalhar, é uma linha. **Quatro suítes mudaram de lado, e a mudança é dela e não minha** — `escala`, `escala-como-ele-descreveu`, `registro-protegido` e a da folha das ATAS na fase anterior. *Teste que guarda regra revogada é pior do que teste nenhum: ele impede a mudança que alguém decidiu, e a frase que ele cobra vira a justificativa para não fazer.* **E duas frases envelheceram junto, as duas reescritas:** o aviso de quem alcança o bloco protegido nomeava dois cargos, e a folha de retirar dizia *"— opcional"* ao lado do motivo. *No protótipo o bloco protegido NÃO EXISTIA: as duas ocorrências semeadas eram de categorias não restritas e nasciam com a fala espontânea nula, então a regra mais cuidadosa do sistema não tinha como ser mostrada a ninguém (§6.19 pela oitava vez). Agora há uma, curta e contida — ela existe para a equipe ver COMO o registro se comporta, e não para ilustrar um caso —, e o ensaio percorre os dois lados: o Líder Diurno lendo, e o educador recebendo a frase que diz a quem pedir.*
| 145 | **Todos leem a ATA coletiva — e quem não lê a linha restrita passa a poder PEDIR.** Decisão da Fundação em 22/09, em duas partes: *"todos leem a ata coletiva, seja manhã ou noite, para consultar informações de como os atendidos estavam — menos as restritas, que são apenas para quem tem autorização. A pessoa pode solicitar ler alguma coisa, e cabe à equipe deixar ou não."* **É a §10.2 respondida**, e por um lado que eu não tinha previsto: a 1510 deixou a ATA Geral fora de quem não a lia justamente para não responder de carona uma pergunta da Fundação, e ela respondeu. **Parte 1 — quem faltava era a Enfermagem, e o caso dela é o melhor exemplo:** ela chega às 9h e precisa saber como a criança passou a noite — se dormiu, se comeu, se vomitou a dose —, e esse dado está na ATA do noturno. A **linha desta casa na ATA Geral** passa a aparecer para educador e Enfermagem também; *o que continua fechado é a FOLHA COMPLETA das oito casas*, que é outro documento e outra rota. **Cozinha e `admin_tecnico` ficam fora, e não por esquecimento:** a cozinha recebe papel, como a portaria, e a ATA traz o dia inteiro de vinte crianças para responder a uma pergunta sobre restrição alimentar que as três folhas dela já respondem; o `admin_tecnico` é conta técnica. **Parte 2 — o pedido, e a decisão de desenho que o resto segue: ele é PELA ATA, e não pela linha.** Quem não alcança a linha restrita **não sabe qual linha é** — a tela lhe diz apenas *"há 2 observações restritas"*, pela `app_ata_restritas`; oferecer uma lista de linhas para escolher seria o vazamento que a restrição existe para impedir. **A liberação é por ATA e por pessoa, e é revogável** — liberação que não se retira é ampliação permanente de acesso pela porta dos fundos. **As duas respostas pedem motivo:** negar é o que a pessoa vai perguntar, e **liberar é o que alguém vai perguntar pela criança**. Ninguém libera o próprio pedido; nada se sobrescreve, e depois de uma retirada pedir de novo cria linha nova, porque a situação muda. *A aplicação não escreve na tabela — `INSERT`, `UPDATE` e `DELETE` revogados de `rede_app` —, e os motivos não vão para o log: eles dizem o que alguém precisa saber sobre uma criança (§5).* **A porta de pedir fica COLADA na frase que diz que existe algo que não se lê**, e não numa aba à parte: é nesse instante que a pessoa sente a falta. E a fila de quem decide fica no topo da aba da casa — pedido esperando numa aba que ninguém visita é pedido que morre, e aí a pessoa vai à coordenação pelo corredor, que é o caminho que este sistema existe para aposentar. **Três achados de desenho, e os três são lições que a casa já tinha.** *(1)* A lista de quem lê a ATA Geral existia em TRÊS lugares — banco, serviço e tela — e eu ampliei só o do banco: por meia hora o protótipo dizia a verdade e o produto dizia o contrário. Agora há **teste que cobra as três iguais**, lendo o catálogo e os dois arquivos de TypeScript. *(2)* Eu escrevi um `FolhaMotivo` novo e o arquivo **já tinha um** — foi o `tsc` que disse; o que existia ganhou o piso e o exemplo como parâmetro, porque o pedido pede dez caracteres e a reabertura da ATA pede quinze. *(3)* A suíte não consegue desfazer o que cria: a `ata` tem gatilho que recusa DELETE e a `ata_note` é IMUTÁVEL — **as duas regras estão certas**, e desligar um gatilho de integridade para limpar teste seria mexer no que guarda o registro da criança. A fixação foi para **quatrocentos dias atrás**, fora de toda janela de consulta: é o contrário da lição da fase 127 — lá a suíte tinha de FECHAR o que abria; aqui ela não tem como, e a saída é não pôr o que abre no caminho de ninguém. *E duas cobranças do ensaio mudaram de lado em dois dias seguidos: a aba da ATA Geral mandava "use o Arquivo", o que era mentira para o educador na 1510 e voltou a ser verdade na 1540. Frase de tela que envelhece é frase que mente — e a cobrança que a guarda envelhece junto.*
| 146 | **A contenção física podia ser escrita na ocorrência de outra casa — e o achado veio de uma medição nova.** Com o §9 vazio, a pergunta passou a ser *"o que nunca foi exercitado?"*, e ela tem resposta medível. Depois de a suíte inteira rodar — oitocentos e noventa e dois testes, naquele dia —, dezenove tabelas continuavam VAZIAS. Vazio no fim, porém, não distingue *"ninguém escreveu"* de *"a suíte limpou"*, e as suítes desta casa limpam porque o banco é um só: quem separa os dois é o `pg_stat_user_tables.n_tup_ins`, que conta as inserções da rodada **inclusive das linhas apagadas depois**. Treze tinham sido escritas; **seis nunca receberam uma linha em teste nenhum**. Quatro dessas estão vazias por DECISÃO ESCRITA e não são defeito — `work_schedule` (MORTA), `medication_authorization` (dormente desde 08/09) e o `medication_protocol` com o seu histórico, cujas rotas de escrita saíram naquela mesma decisão; *eu suspeitei de defeito no protocolo e a leitura do código me corrigiu: ele é história, de propósito.* **Sobraram duas superfícies, e uma tinha buraco.** A inserção da CONTENÇÃO exigia apenas que quem assina fosse quem está logado — **nada exigia que a ocorrência estivesse no alcance de quem escreve** —, e `addRestraint` era o único dos irmãos que não conferia a casa no serviço (`addSynthesis`, `addProtected` e os anexos conferem). A chave estrangeira não ajuda: ela é conferida como DONA da tabela, por fora do RLS. **Medido: uma conta de outra casa, com o uuid na mão, escrevia a contenção — e o efeito é pior do que o vazamento.** A tabela tem chave primária no `incident_id`, então a linha do estranho OCUPA o lugar e **impede a contenção verdadeira de ser registrada**: o educador que conteve a criança às 3h recebe *"esta contenção já foi registrada"*. Contenção é o registro mais grave que este sistema guarda sobre uma criança — o que foi feito com o corpo dela, por quem, por quanto tempo —, é append-only, e uma linha falsa aqui não se corrige depois. **A correção é nos dois lugares:** a política, que vale para qualquer caminho novo, e o serviço, para a recusa chegar em português (regra 8 — fora do alcance responde igual a inexistente) em vez de em inglês de Postgres na tela de quem registra às 3h. *Medida sem a correção, a suíte nova reprova em **seis dos oito**.* **A outra superfície estava CORRETA** — a saída de medicamentos com o acolhido confere a casa, o cargo e tem índice único; o que faltava era alguém provar, e ela importa porque é o remédio que a criança leva para o fim de semana. *Exercitá-la revelou uma ordem que ninguém tinha notado: o serviço confere "já registrada" ANTES do cargo, então o guarda de cargo daquela rota nunca era alcançado — a ordem está certa ("já foi feito" é resposta mais verdadeira que "você não pode", e não conta nada a ninguém), e o teste agora o alcança numa saída nova.* **E a medição virou script** — `scripts/superficies-sem-teste.mjs` —, pela razão da fase 140: medição feita uma vez é medição que apodrece. *Ele tem uma guarda que é a parte mais importante dele:* rodá-lo depois de UMA suíte devolve oitenta e três candidatas, todas falsas, porque o `globalSetup` recria o schema e zera o contador — foi o que aconteceu na primeira vez que eu o rodei. **Medidor que mente quando mal usado é pior do que medidor nenhum**, e a saída dele tem a mesma cara nos dois casos; então ele confere a PROPORÇÃO de tabelas tocadas e se recusa a concluir abaixo dela. *E um falso verde meu, o terceiro desta sessão: o teste da política passava sem a correção — não pela política, mas porque a chave primária já estava ocupada pela linha que o buraco deixara entrar. Agora ele usa uma ocorrência sem contenção, e cobra que a mesma escrita PASSE para quem alcança a casa, senão estaria provando apenas que algum erro acontece.*
| 147 | **Das 342 rotas do servidor, 28 nunca tinham sido chamadas por teste nenhum — e oito eram a família do defeito da fase 140.** A medição da 146 tem um irmão: lá a pergunta era *"que TABELA nenhum teste escreveu?"*; aqui é *"que ROTA nenhum teste chamou?"* (`scripts/rotas-sem-teste.mjs`). **A família que importava era a do vocabulário** — as oito rotas que entregam à tela a lista de opções que ela oferece. O §12.2 diz que a tela nunca inventa a própria lista, e a 140 mediu o custo de a lista discordar de onde o valor é guardado: a folha da Evolução oferecia `vacina`, que o enum não tem, e **quem registrasse uma vacina recebia 500** — enquanto `emergencia` e `terapia` eram aceitos e nunca oferecidos. *Nenhuma das oito tinha teste.* A cobrança agora é nos DOIS sentidos e vem do CATÁLOGO: onde o valor mora numa regra `CHECK` — compromisso, recorrência, reunião, marco de vida, vínculo do contato — o conjunto oferecido tem de ser **igual** ao aceito, e não "contido", que passaria com a metade faltando; e as opções de testemunho **SÃO a tabela** `witness_option`, comparadas na ordem dela, porque a ordem decide o que a educadora vê primeiro às 23h. **Resultado: as oito estão corretas e alinhadas.** Não havia defeito ali — o que não havia era a garantia, e agora há. **E dois atos da lista ganharam teste por serem os que mais custariam errados.** A troca da própria senha, lida antes de testada, está correta: pede a senha ATUAL, exige que a nova seja diferente, e **encerra as outras sessões abertas menos a de quem trocou** — a linha que importa numa casa onde o computador da sala fica ligado, porque quem troca a senha por desconfiança precisa que as outras caiam e precisa continuar trabalhando na dele. *A suíte guarda o hash antes e o devolve no fim: a senha de desenvolvimento é a mesma em todas as suítes, e deixá-la trocada derrubaria o login de todas as que rodam depois — a lição da 127 com outra roupa.* E as duas rotas do aviso, porque **ler não é dar ciência** (§8.5): *ler* é "eu vi", *dar ciência* é "eu assumo", e a caixa de entrada da coordenação depende de serem coisas diferentes. **O medidor teve de ser consertado pela minha própria suíte.** A primeira versão só via a chamada com o método colado à URL, e a suíte de vocabulário chama por um auxiliar — recém escrita, as oito rotas dela continuavam aparecendo como "sem teste". Ele passou a separar **candidata forte** (a URL não aparece em suíte nenhuma) de **par incerto** (a URL aparece, o método não casou), e sem essa separação ele era quase inútil: catorze das vinte e cinco eram ruído. *Rodei também o ensaio de produção, que não rodava desde antes das seis migrações desta sessão: o sistema sobe compilado num banco virgem, as migrações aplicam pelo binário, e ele continua recusando subir com a conexão que passa por cima do RLS.*
| 148 | **Sete de dez rotas que recebem data devolviam 500 para uma data que não é data.** O achado veio de seguir a lista da 147: a primeira rota que eu abri — `PATCH /shifts/general-night-line/:data/house/:houseId`, a que a **coordenação e o Líder Diurno** usam para corrigir a linha da casa deles na ATA Geral — respondeu **500, "Internal server error"**. A data chegava do `@Param` sem conferência nenhuma e ia direto para um `::date` do Postgres. *E ela é justamente a rota que a 1440 criou para o identificador da folha das oito casas NÃO sair do servidor — a suíte da fase 138 exercita a outra, a que leva o id, e esta nunca tinha sido chamada.* **Medi o alcance antes de escolher o conserto**, e o alcance foi o que decidiu o desenho: dezesseis pontos do servidor recebem data, nenhum conferia, e das dez que sondei **sete devolveram 500** — a agenda, a escala, a folha da escala, as compras de medicamento, os pedidos da cozinha, o resumo deles e o arquivo das ATAS. **Quem cai nisso**, porque *"a tela manda data boa"* não é resposta: um link guardado de outro mês, um endereço digitado à mão, um campo de data do celular que envia metade do valor, uma fila offline que sobe um parâmetro truncado. Quem vê o 500 é quem está de plantão, e ele não diz o que fazer. **A correção é UM lugar** — `DataDoDia`, no kernel, aplicado aos vinte e nove parâmetros de data do servidor —, e é a lição da 141: regra nova sobre data se escreve uma vez, não em dezesseis lugares dos quais quinze vão esquecer. *Ele recusa duas coisas:* a FORMA errada, e **o dia que não existe no calendário** — `2026-02-30` passa pela forma, e o `Date` do JavaScript a "conserta" virando 2 de março, que é como uma consulta responde sobre outro dia sem avisar ninguém. *E ele NÃO inventa a data de hoje:* vazio passa como vazio, porque quem decide o que é hoje é a instituição, no banco (§23). **A suíte varre TODAS as rotas de data, e não as que eu sondei:** ela lê os controladores e cobra que todo `@Query('de')` e `@Param('data')` passe pelo pipe — uma lista escrita à mão envelheceria na primeira rota nova, e a rota nova é exatamente a que vai esquecer. Medida sem a correção, **quatro dos cinco testes reprovam**, e o que passa é o que tinha de passar: a data boa continua entrando. *E uma correção de frase minha: a lista das catorze rotas, escrita ontem no §9, dizia que esta rota era "a escrita original" e que a correção tinha suíte. Era ao contrário.*
| 149 | **A coordenação lia metade do rastro da própria casa — e a metade que faltava era a que mais importa.** O achado veio de seguir a lista do §9: a próxima rota sem teste era `GET /audit/report/:id`, *"por onde esta cópia saiu, e para quê"*. **Li os dois lados antes de escrever o teste**, como a 145 ensinou, e a hipótese que eu tinha era outra — que algum cargo passasse pelo guarda do serviço e fosse recusado calado pelo RLS. Não era: `LEEM` e a policy `audit_select` nomeiam os MESMOS três cargos. **O defeito estava do outro lado da mesma linha.** A `audit_select` (0920) dá à coordenação o que está na casa dela — `house_id = ANY(app_casas_no_alcance())` —, e **`NULL = ANY(...)` é NULL, que não é verdadeiro**: linha de auditoria que nasce sem casa é invisível para quem responde pela casa. **Medido no repositório inteiro: das 152 chamadas de `audit.log`, 79 não passavam `houseId`.** O que isso queria dizer na mesa da coordenadora da Casa 03: o relatório **judiciário** de uma criança dela era gerado, aprovado, exportado em Word e entregue ao Ministério Público, e a auditoria dela respondia *"nada aconteceu com este registro"* — porque só o `report.generate` de relatório DE CASA levava casa, e relatório de criança nem esse, já que o formulário dele não tem unidade. Com ele: a leitura do dado bancário e a tentativa negada dela, o anexo da internação aberto, a foto de identificação guardada, a memória da criança aberta, a dose confirmada, a situação judicial consultada, o combinado revogado, a retirada da escala. **Tudo gravado, e nada legível para quem responde pela casa** — o §7 promete *"a coordenação lê o rastro da própria casa"*, e o sistema cumpria metade dela. *E a auditoria é append-only: o que nasceu sem casa fica sem casa. A correção vale de hoje para frente, e é por isso que ela não podia esperar outra fase.* **Escolhi corrigir nos 63 pontos de escrita, e não num gatilho do banco** — que era o caminho de "um lugar só" e chegou a ser o meu primeiro desenho. Dois motivos o derrubaram: um gatilho que deriva a casa do estado de HOJE daria a casa errada para o ato de ontem (a criança que mudou de casa, a transferência, o desligamento), e um `CASE` sobre cinquenta tabelas dentro do módulo que é dono da `audit_event` faria a base depender do topo — remover uma partição passaria a quebrar a auditoria, que é justamente o que o `arquitetura.spec.ts` existe para impedir. **O lugar único é a REGRA, e ela virou conferidor:** `auditoria-tem-casa.spec.ts` lê o servidor inteiro, casa as chaves de cada chamada e cobra `houseId` em todas, com uma **lista de exceção ESCRITA** — a sessão de quem entra, o cadastro da equipe, a consulta de CPF na admissão, o que é das oito casas de uma vez. *Lista de exceção escrita é a diferença entre uma regra e uma lembrança*, e a terceira cobrança é que ela não envelheça: ação que sai do código sai da lista. **Três respostas passaram a existir no kernel, ao lado de quem ESCREVE a auditoria** — a casa de um acolhido (com a ÚLTIMA casa quando não há atual, porque criança desligada continua tendo história), a casa de um registro pela tabela dele, e as duas do módulo de medicamentos. As três **leem como DONAS**, de propósito: a linha tem de nascer com casa mesmo quando quem age não alcança a criança, e a tentativa negada de ver benefícios é exatamente esse caso. *Nada delas volta para a tela — o valor entra na coluna `house_id` e só, e o nome da tabela vem sempre de uma lista de constantes do servidor, nunca de fora.* **Duas coisas que a correção encontrou de passagem.** A exportação do relatório gravava a finalidade no `export_log` e **não** na linha de auditoria, numa tabela que esta rota não consulta: a auditoria devolvia a exportação **sem o para quê**, que é a única pergunta que alguém faz seis meses depois. E o `marco.registrado` teria ficado sem casa mesmo com a correção, porque a linha é lida de outra conexão enquanto a transação que a criou ainda não fechou — ali a casa vem do `RETURNING`, e é a casa GRAVADA no marco, não a de hoje. **Medida sem a correção, a suíte nova reprova em três dos oito** — e os cinco que passam são os que tinham de passar: o recorte não vazava nada para a casa vizinha, a recusa em português chega a educador e Enfermagem, e o Gestor Geral lê as oito. **E o falso verde desta fase foi meu, é o quarto da sessão, e é a lição que eu levo dela.** As três respostas novas liam a casa pelo `db.query` do kernel — a consulta SEM identidade. **Sem identidade o RLS não devolve linha nenhuma:** `SELECT count(*) FROM house_stay` como a aplicação sem usuário dá **zero**, medido. Ou seja: as cinquenta e três correções gravavam `null` do mesmo jeito, e **as duas cobranças que eu tinha escrito passavam** — a suíte de ponta a ponta porque o relatório lê a casa por outro caminho, e o conferidor estático porque ele cobra que o `houseId` esteja ESCRITO, não que ele chegue ao banco. *Conferidor estático e teste de ponta a ponta podem estar os dois verdes sobre uma resposta que responde `null`*, e o que os separa é um teste que lê o dado DE VOLTA pelo caminho de quem pergunta. Ele existe agora: a técnica escreve no perfil da criança e a coordenação lê o ato em `GET /audit/person/:id`, com a casa. *Medido sem a correção, ele reprova.* **A leitura passou a ser sob o RLS de quem agiu**, e isso é mais verdadeiro do que ler como dona: quem age alcança a criança de quem age — inclusive nas recusas que mais importam registrar, porque o educador que tentou abrir dados bancários é da casa dela, e o que lhe falta é a permissão daquela área. Quem está fora da casa recebe `null`, e aí a linha sem casa é a resposta certa: para a casa, aquele ato é de um estranho, e é a gestão geral que o lê. **E a correção derrubou uma cobrança antiga, pelo melhor motivo:** a suíte do trabalho da equipe (117) exigia que os quatro recortes do painel não contivessem a palavra *"acolhido"* nem *"criança"* — e passou a reprovar no relógio adiantado, porque linhas que antes ninguém lia chegaram ao painel e trouxeram os NOMES delas: *"Cadastro do acolhido alterado"*, *"Relatos de uma criança lidos no perfil dela"*. **Não é defeito: a ação é sobre uma criança, e o vocabulário existe para dizer isso em português.** O que a regra 3 proíbe é CONTAR por criança, e isso está na DIMENSÃO do recorte, não no rótulo — então a cobrança passou a fixar os campos por extenso, o que é mais forte: recorte novo por criança reprova mesmo que ninguém o chame de criança. *Ela só passava porque as linhas que nomeiam criança não chegavam ali — e fazer com que cheguem é o conserto desta fase.* |
| 150 | **A moldura do sistema estava amadora — e o pedido veio com o exemplo do concorrente na mesa.** *"Até o nosso layout é muito amador; quero algo mais bonito, mais vivo e colorido... até os emoji e botões são muito feios; acredito que na lateral ficaria melhor."* **Antes de desenhar eu ABRI o que temos**, no Chromium, nas mesmas telas dos exemplos e nas duas larguras — porque opinar sobre tela sem olhar é como se troca o que funciona pelo que parece melhor. O diagnóstico teve quatro itens, e três eram de fato defeito. **(1) A barra do topo era um bloco navy de ponta a ponta:** medido no celular, **190px — quase um terço da tela** — gastos com nome, e-mail e sete botões antes da primeira linha do dia, que é onde está a criança de quem a educadora precisa saber. **(2) A navegação era emoji.** E o problema deles não é gosto: *cada sistema desenha o seu* — o 🧒 é um rosto no Android e outro no iPhone da casa, o 🗓️ aparecia em três telas com três desenhos —, *eles não herdam a cor* (são imagem colorida de fábrica: não escurecem no tema claro, não clareiam no escuro, não acompanham o item ativo, e é isso que dava o ar de adesivo colado por cima), e *carregam significado que ninguém pediu* — o 🧒 diz "criança pequena" numa casa que acolhe adolescentes de dezessete anos, e o 🎯 de "lazer e atividade" é um ALVO, a coisa mais distante do que uma tarde de futebol significa para eles. **(3) No monitor havia uma coluna de conteúdo esticada e nada na lateral**, com metade da largura vazia. **(4) O que NÃO era defeito, e por isso não mudou:** o sistema de cores já era sério — tokens, tema escuro, contraste medido —, e a barra colorida do cartão continua sendo a CATEGORIA. **As duas escolhas eram da Fundação e ela respondeu: a moldura fica CLARA**, com o navy no logotipo, no item ativo, nos títulos e nos botões principais — marca se vê sem atrapalhar o turno —, e **a cor de pessoa será por CARGO, não por indivíduo** (é a fase seguinte). **A navegação passou a ter DUAS FORMAS, e é a MESMA navegação:** no celular a barra de abas de sempre com a folha "Mais", porque é o que cabe no polegar de quem está no corredor às 23h; no monitor, a partir de 1080px, uma COLUNA à esquerda com as vinte e cinco portas abertas e agrupadas — o `nav.tabbar` não foi trocado por outro elemento, ele muda de eixo, e isso foi decisão consciente: os ensaios entram por ele, e trocá-lo deixaria a navegação sem conferência exatamente na fase que a reescreve. **Trinta e nove desenhos de traço** entraram no lugar dos emoji da moldura e das categorias do dia, em `currentColor`, sem arquivo para buscar — a entrega continua sendo um HTML só que abre sem internet. *Nenhum deles é o único portador do significado: ao lado há sempre a palavra, e é ela que o leitor de tela anuncia.* **E a fase achou o que não estava procurando, que é o padrão desta casa: a lista de telas existia em DOIS lugares** — um vetor de vinte e três chaves e vinte e cinco blocos escritos à mão —, e **as duas discordavam**: a rotina da casa e a escala de plantão estavam na folha e fora da contagem. O efeito hoje era invisível (outra porta já deixava o "Mais" verdadeiro), mas no dia em que fossem a única porta de um cargo, a pessoa ficaria sem o botão que abre a tela. Agora é uma tabela só (`frontend/src/portas.ts`), e **o texto dela não foi redigitado: foi extraído do que já estava na tela, em código** — redigitar rótulo de memória é como se inventa nome de tela. **Duas conferências nasceram junto.** O `as-portas-e-os-icones.spec.ts` cobra que toda porta tenha desenho que existe (nome errado não dá erro nenhum: o `Icone` devolve nada de propósito, e a porta fica com um buraco ao lado que ninguém nota num menu de vinte e cinco linhas), que tenha título e a frase que a distingue da vizinha, que aponte para tela que o `App` conhece, e que a moldura não volte a ter emoji. *Ele me corrigiu duas vezes na primeira rodada:* reprovou nas vinte e cinco portas porque eu cobrava um `setAba('x')` escrito que a tabela justamente aposentou, e reprovou por emoji **no comentário que explica por que os emoji saíram** — proibir a palavra no texto que explica a proibição é a forma mais boba de um conferidor virar estorvo, porque quem tropeça nele apaga a explicação. **E o `npm run ensaio:largo`**, que roda o percurso inteiro a 1440px: a coluna nasceria sem ensaio nenhum, porque os seis rodavam a 420px, onde ela não existe — é a lição da 141 (superfície sem ensaio é superfície sem teste) aplicada antes de doer. *Medido: 139 telas sem violação de WCAG 2.1 AA com a moldura clara, e os sete ensaios verdes.* **E um acoplamento ruim que a troca revelou:** o `ensaio:uso` achava a chave do trabalho social procurando o emoji 🌱 DENTRO do botão. Sem o emoji ele não achava a chave, a tela nunca abria, e **três cobranças passaram a reprovar sem que nada estivesse errado com elas** — as três sobre não haver ranking de casas, que são das mais importantes que este sistema tem. *Prender um ensaio ao DESENHO de um botão é prendê-lo à decisão mais volátil de uma tela;* ele passou a entrar pelo NOME ACESSÍVEL, que é o contrato de verdade — muda quando a função muda, e é o que a pessoa com leitor de tela ouve. **O que ficou para a 151, dito por extenso para não virar promessa:** 107 emoji continuam nas TELAS (o 💊 do botão "Remédios", o 📝 do "Relato"), e a cor por cargo ainda não existe. |
| 151 | **A outra metade do pedido visual: a cor por cargo, e os emoji que ainda estavam dentro das telas.** A 150 trocou a MOLDURA; sobravam **107 emoji em vinte arquivos** e a cor que a Fundação escolheu em 23/09. **A COR POR CARGO TEVE DE CABER ENTRE DUAS QUE JÁ EXISTEM, e é isso que a torna delicada.** *(1)* A **cor de ESTADO** — vermelho crítico, âmbar em atenção — responde *"isto ainda precisa de alguém?"*, é a mais importante da casa, e por isso **o cargo NÃO usa a família dela**: um círculo âmbar ao lado de um nome seria lido como *"esta pessoa está em atenção"*, que é exatamente o que a §6 proíbe — cor comunica estado operacional, nunca julgamento sobre a pessoa. O cargo é fundo CHEIO e escuro com letra branca; o estado é pílula clara com letra colorida: formas diferentes, para não se confundirem de relance. *(2)* A **cor de AUTOR** (`tomDoAutor`, fase 123) responde *"qual colega escreveu esta linha?"* na ATA e na escala, e a coordenação a escolhe por pessoa desde a 0990 justamente para dois educadores do mesmo plantão não caírem no mesmo tom. **O cargo não a substitui** — trocá-la pintaria os dois de igual, que é o defeito que ela existe para consertar. *(3)* O cargo responde a uma TERCEIRA pergunta, que é a da escala e a do painel do plantão: *"de que setor é esta pessoa?"* — quem olha a semana quer saber se há técnica no turno, não qual das duas técnicas. Na linha da escala as duas convivem de propósito: **o círculo é do cargo, a borda é da pessoa.** **Nove cargos, todos conferidos contra branco** — o menor é o verde do educador, com 4,9:1 —, e as famílias foram escolhidas para não colidirem na mesma tela: os cinco que aparecem juntos na escala estão em azul, terra, violeta, verde e turquesa, e nenhum par é verde-e-vermelho. *O âmbar do Líder Diurno é o `#92400E` da fase 118, e não o `#B45309` que pareceria natural — aquele reprova por quatro centésimos.* **E a cor nunca anda sozinha:** o círculo carrega as iniciais, e o nome do cargo por extenso vai no rótulo acessível — quem não distingue os tons, quem imprime em preto e branco e quem usa leitor de tela recebem a mesma informação. **Os 107 emoji saíram, e a varredura foi feita em código com os comentários MASCARADOS** — trocar dentro do texto que explica a troca é como se apaga a explicação (lição da 150). Vinte e seis desenhos novos entraram; dez casos não casaram com o padrão e foram à mão, porque o emoji estava DENTRO de uma string: um ternário que escolhia entre `'🥪 Lanche'` e `'🧺 Cesta básica'`, o selo da fila offline, o `'👁 Abrir'` do dossiê. **E um deles não era ícone nenhum: era uma FRASE que envelheceu** — a tela da senha dizia *"troque pelo botão 🔑 na barra"*, e sem o emoji a frase apontava para nada; passou a nomear o botão por extenso. **Regra 14 pegou esta fase no flagrante:** o ícone de cada conquista (passou de ano, primeiro emprego) vem do **servidor**, e o `mock.ts` o espelha — trocar num lado só faria o protótipo que o Marcelo abre desenhar certo e o sistema de verdade desenhar errado, que é o pior jeito de errar, porque a demonstração fica bonita. Os dois foram trocados juntos, e **há conferidor que os compara linha a linha**. *A conferência da 150 cresceu: ela guardava a moldura, e agora varre as trinta e seis telas inteiras — emoji volta com um toque, e ninguém repara numa tela entre trinta e seis.* **Medido: 139 telas sem violação de WCAG 2.1 AA com a paleta nova.** |
| 152 | **O nome de quem deu o remédio sumia para quem mais precisava lê-lo — e o cargo de quem escreveu cada linha da ATA, também.** A fase voltou à lista do §9, e a primeira rota era `GET /medications/prn`: o remédio *"se necessário"*, o que o educador dá às 2h da manhã. **Li antes de testar**, e o suspeito apareceu na leitura: a lista do que JÁ foi dado hoje buscava o nome de quem deu por `LEFT JOIN app_user` — e `app_user` tem RLS por linha: **educador, Líder Diurno e Enfermagem só leem a PRÓPRIA.** A dose que a educadora da noite deu às 2h chegava à Enfermagem, de manhã, **sem nome de quem deu** — e o comentário do próprio serviço diz que esta lista existe para isso: *"é assim que a Enfermagem a lê na manhã seguinte"*. **O defeito era silencioso por três camadas:** o `LEFT JOIN` devolvia vazio sem erro; a tela escreve *"por Fulana"* só quando o nome vem, e apagava a frase inteira quando não vinha; e **o `mock.ts` preenchia o nome sempre** — o protótipo mostrava o que o produto escondia, e quem abria a demonstração via a versão certa. *Medido: a suíte nova reprova em dois dos sete sem a correção — exatamente a Enfermagem e o Líder Diurno.* **A correção é a função que a casa já usa para isto desde a 0060**, `app_user_display_name`: devolve o nome e nada mais, e por isso pode atravessar o recorte sem abrir o cadastro. **E a pergunta que a fase obriga é "em que outro lugar um nome some do mesmo jeito?"** — medida no servidor inteiro, e ela achou dois irmãos, os dois em `shifts.service.ts`. **O pior era na ATA:** o NOME de quem escreveu cada linha sempre veio pela função, mas o **CARGO vinha por subconsulta direta**, sob o RLS de quem lê — e a educadora que abria a ATA de ontem para saber como a casa passou a noite lia **"— · 02:10"** na linha da coordenadora, onde devia ler *"Coordenação · 02:10"*. É a leitura que a Fundação pediu com todas as letras na conferência de 22/09 (*"cada educador pode ver uma ata unificada da passagem dos dias anteriores"*), e ela chegava a quem a pediu com um buraco em cada linha alheia. O mesmo acontecia no pedido de leitura da observação restrita (1540), para o Líder Diurno que decide. **Nasceu a irmã da função de nome — `app_user_cargo` (migração 1560)**, com a mesma forma: um campo, o código do cargo, e nada mais; sem `p_house`, e por isso sem `app_house_in_scope`, exatamente como a irmã; `search_path` por extenso. *Medido: sem ela, a suíte da ATA reprova no caso de quem LÊ e passa no de quem escreveu — que é precisamente por que ninguém via: quem escreve sempre viu o próprio cargo.* **E de passagem fechou o item anotado desde a 149:** a grade do remédio e a lista do "se necessário" perguntavam a ausência da criança como PAR — internada **e** com a família — em vez de `app_ausente_da_casa`. O efeito era idêntico; o risco era a lição da 141 de novo — a próxima ausência escrita no lugar único não chegaria ao remédio, e a dose de quem não está na casa voltaria a aparecer como "aguardando confirmação". As duas razões continuam escritas no comentário; a pergunta passou a ser uma só. *A suíte nova abre uma convivência familiar e a FECHA no fim — a lição da 127, porque uma criança fora da casa mudaria a chamada de todas as suítes seguintes.* **E a pergunta que sobrava era a mais incômoda: por que o `arquitetura.spec.ts` não pegou?** Ele existe exatamente para isto — *todo JOIN com tabela protegida por RLS está justificado* — e a resposta estava na forma dele: bastava existir um `rls-join-ok` **nas quatro linhas de cima**. A justificativa escrita para o `JOIN prescription` estava a três linhas do `LEFT JOIN app_user`, e **isentou os dois — sendo falsa para o segundo**. Uma justificativa passou a valer só para a tabela que ela NOMEIA. Medido ao endurecer: oito outras justificativas não nomeavam a tabela — **as oito estavam certas**, escritas com o nome em português (*"a casa"*, *"a prescrição"*), e ganharam o nome da tabela; a única isenção falsa era a da 152. **E o da ATA ele nunca teria pegado:** era SUBCONSULTA, não JOIN, e a regra só olhava JOIN. Ela passou a olhar as duas formas — é a mesma pergunta com outra sintaxe. *Medido nos dois sentidos: com cada defeito recolocado no lugar, a regra reprova apontando a linha; sem ele, passa.* |
| 153 | **A chamada, o plantão, as atividades e a grade do remédio devolviam 500 para uma data que não é data — cinco fases depois de eu ter consertado isto.** A lista do §9 seguia em `GET /timeline/all`, e **a primeira linha do controlador já dizia o que havia**: `@Query('date') date?: string`, sem o `DataDoDia` que a fase 148 criou para toda data que vem da URL. **Por que o conferidor da 148 não pegou:** ele procurava os nomes `data`, `dia`, `de` e `ate` — **em português** —, e o parâmetro se chamava `date`, em inglês. Era a lição que eu mesmo escrevi naquela suíte, *"lista escrita à mão envelhece"*, e a lista era a DO CONFERIDOR. **Medido no servidor inteiro, pelo inventário de todo parâmetro de URL:** dez `@Query('date')` sem conferência, e **sondadas, quatro devolviam 500** — `GET /checks` (a chamada), `GET /shifts` (o plantão), `GET /activities` e `GET /medications` (a grade do remédio), **as quatro telas mais abertas do turno**; e a linha do tempo respondia **200 sobre um dia qualquer**, o que é pior de outro jeito: responde sem avisar. Mais o `?hora=` da agenda, que ia direto para um `::time` do Postgres — *"quem está na escala às 14h?"* com `14h` devolvia 500 a quem marcava uma consulta. **E um falso verde meu, da própria 148:** a sondagem dela chamava `/shifts?data=nao-e-data` — mas a rota lê `date`; ela ignorava o parâmetro, respondia sobre hoje com 200, e a cobrança passava **sobre um parâmetro que a rota nem lia**. **A correção** aplica o `DataDoDia` às dez, e ganha uma irmã no mesmo arquivo, **`HoraDoDia`** — conferência do que chega pela URL mora num lugar só. **E o conferidor passou a NEGAR POR PADRÃO:** todo parâmetro de URL do servidor tem de estar CLASSIFICADO — data, hora, conferência própria (o `mes`, medido: já respondia em português) ou *não é tempo* —, e nome novo que ninguém classificou reprova com a frase dizendo o que fazer. Um `dataInicio` amanhã não passa por fora só porque ninguém lembrou. *Medido nos dois sentidos: sem o pipe da chamada ele reprova apontando a linha; com um parâmetro inventado sem classe, também.* A sondagem passou a exigir **400**, e não "qualquer coisa menos 500" — a linha do tempo respondia 200 para lixo, e uma cobrança que só proibisse o 500 teria passado por ela. **`GET /timeline/all` estava certa por dentro, e ganhou teste:** a leitura levantou a suspeita de que o educador receberia as oito casas com "0 eventos" — o que diria *"nada aconteceu lá hoje"* sobre casas que ele não alcança —, mas `house_select` é `app_house_in_scope(id)`; o teste guarda que continue assim. **`GET /transfers/pending` saiu.** Ela estava na lista das rotas sem porta de tela com a justificativa *"contagem de transferências à espera, para aviso"* — e devolvia a **lista**, não uma contagem, e **ninguém a chamava**: nem tela, nem aviso, nem teste, nem o servidor de mentira. Era um invólucro *"compatível com a chamada antiga"*, e a chamada antiga não existia mais. Frase de exceção que envelhece é frase que mente. |
| 154 | **A ficha de saúde e a trajetória da criança saíam do sistema sem que a coordenação da casa soubesse quem as tirou.** A fase nasceu de uma conferência pedida pela Fundação em 25/09 (*"confira se todo o sistema continua completo"*): 1007 testes nas duas condições de relógio, sete ensaios, 139 telas, protótipo idêntico, nenhuma tabela sem exercício — **e uma sondagem nas nove rotas que o §9 ainda listava.** Três achados, todos em código que compilava e passava. **(1) As duas exportações POR CRIANÇA gravavam a auditoria SEM CASA** — `exportarSaude` e `exportarTrajetoria` não passavam `houseId` ao `documentos.exportar`, e linha sem casa é linha que a coordenação não lê (149). **O conferidor da 149 não pegou porque olhava só as chamadas de `audit.log`**; a exportação grava por dentro do `documentos.exportar`, que passa adiante a casa que recebe. **(2) A folha do remédio que vai com a criança gravava a casa que a TELA mandava**, e não a da ida, e não dizia qual ida tinha saído (`entidadeId` vazio). **(3) Casa ou data inválida no CORPO derrubava sete exportações com 500** — a escala, os combinados, o estatuto, a grade do remédio, o relatório de período e duas folhas da cozinha, em dezesseis casos. A sondagem de 25/09 tinha visto três; **foi o teste que olhou TODAS as rotas de exportação, tiradas de um `grep`, que achou as outras quatro**. É o defeito da 148 e da 153 pela terceira porta: elas conferiram `@Param` e `@Query`, e nenhuma o `@Body`. **E a leitura das folhas achou um quarto:** as três folhas da cozinha, pedidas por quem não alcança a casa, saíam com o título certo e *"Crianças com restrição: 0"* — **não vazava nada** (medido por cargo, com os ids buscados por fora do RLS: quem é da Casa 03 lê as quatro restrições, quem é da 04 lê zero), mas a folha dizia *"ninguém aqui tem restrição"* em vez de *"não é sua"* (regra 12). **Os consertos:** `casaDoAcolhido` nas duas por criança; a casa da ida sai da `family_stay`, que entrou em `TABELAS_COM_CASA`; o **`CorpoConferido`**, no mesmo arquivo do `DataDoDia` e do `HoraDoDia` — casa tem de ser identificador, dia tem de existir no calendário — em **todas as dezessete** rotas de exportação; e o alcance conferido ANTES do rótulo nas folhas da cozinha. **Dois conferidores estáticos novos:** todo `documentos.exportar` tem de passar `houseId` (medido: sem o conserto da saúde ele aponta a linha), e todo `@Post(...export...)` com corpo tem de usar o `CorpoConferido`. **Medido nos dois sentidos:** com o código de antes, a suíte nova reprova seis de dez; com o de agora, passa. **E dois falsos verdes MEUS durante a própria fase:** a sondagem chamava `/reports/trajetoria/...` e `/reports/export`, que não existem (são `/impacto/...`) — o 404 de rota inexistente passava por recusa. O teste agora recusa `Cannot POST`: é a lição da 153 de novo, *confira que o teste chama o que a rota é*. **E um defeito fora da lista, achado pelo log da suíte:** o **pedido para ler a observação restrita de uma ATA nunca avisava ninguém.** Ele publicava `priority: 'media'`, que o banco não aceita; a notificação era recusada, o barramento escrevia o erro no log — duas vezes por rodada, desde que o pedido existe — e **a técnica e a coordenação nunca eram avisadas**. O contrato `EscalationRequest` sempre disse *"normal, alta ou crítica"*, mas o `publish` era genérico e o compilador nunca comparou. **O `publish` passou a ter o tipo do corpo escolhido pelo NOME do evento**, e o compilador achou esta chamada, e só ela, entre dezesseis. A suíte do pedido ganhou a cobrança que faltava — *o aviso chega à técnica* —, e todas as outras liam a LISTA, nenhuma o AVISO. **E um teste que dependia da hora:** `a-crianca-fora-da-casa-sai-da-grade` reprovou às 00h52 — o item vencido *"de três horas atrás"* caía no dia anterior à internação, e o sistema, certo, cobrava. Nenhum dos dois relógios da suíte cai entre 0h e 3h. A internação do teste passou a começar na véspera. |
| 155 | **Nenhuma escrita cai mais com 500 — e a suíte testava um servidor que não era o que sobe.** A 154 deixou duas medições por fazer, e a fase fez as duas. **A primeira: os erros que só vão para o log.** O barramento engole a falha do ouvinte de propósito (um aviso com defeito não pode desfazer a ocorrência que o publicou), e o pedido de leitura da ATA restrita morreu ali por meses. Agora o barramento guarda cada falha numa lista em memória, e o `test/setup/ouvinte-nao-falha-calado.ts` — um `afterAll` que roda em TODA suíte — reprova quando ela não está vazia, com o evento e o erro. *Medido nos dois sentidos: com o `'media'` da 154 de volta, a suíte do pedido reprova só por ele, mesmo com o teste do aviso desligado.* Nasceu verde nas 106 suítes. **A segunda: o corpo das rotas que NÃO exportam.** Em vez de escolher mais uma porta — a 148 olhou `@Param`, a 153 `@Query`, a 154 o corpo das exportações, e cada uma achou a porta ao lado —, a sondagem passou por **TODA rota de escrita**, lida do código, com corpo vazio e com corpo de lixo: **28 das 174 respondiam 500, em 35 casos.** Vinte e nove eram a MESMA classe — o banco não reconhecia o formato de um identificador, uma data, uma hora ou um número —, e o `FalhasEmPortugues` não a conhecia: quem só preencheu um campo errado lia *"alguma coisa falhou aqui dentro e o que você fez NÃO foi salvo"*. **E o filtro só estava ligado no `main.ts`:** as suítes montam o app pelo `AppModule`, então **rodavam sem ele** — a suíte testava um servidor, a produção servia outro. O próprio teste do filtro dizia isso num comentário, e ligava o filtro à mão só para si. **O conserto:** o filtro ganhou a classe de formato (`22P02`, `22007`, `22008`, `22003` → 400, com frase) e o `no_data_found` (`P0002` → 404; vinte e nove exceções do banco o usam, todas para "não existe", que é a regra 8), e passou a morar no `AppModule` como `APP_FILTER`. **A cobrança nega por padrão:** `a-escrita-que-nao-cai.e2e.spec.ts` lê as rotas dos controladores, e sair dela é uma linha escrita em `FORA` com a razão — hoje a sessão e os seis atos do relógio, que com corpo vazio PROCESSAM o dia. **E a sondagem achou um sucesso que não acontecia:** marcar estoque baixo respondia `ok: true` sempre — para item inexistente e para item de OUTRA casa, que o RLS filtra calado. A Enfermagem lia "sinalizado" e nada mudava, e é essa marca que põe o remédio na lista de compra. Hoje confere quantas linhas mudaram. *Medido o resto da família:* dos treze `UPDATE` dos serviços que não olham o resultado, os outros doze leem a linha antes na mesma transação, ou mexem só na linha da própria pessoa, ou fecham o anterior antes de um `INSERT` que confere o alcance. |
| 156 | **A coordenação de uma casa redefinia a senha de um educador de OUTRA casa — e recebia a senha nova na resposta.** A fase fez as duas medições que a 155 deixou. **As leituras com lixo:** as 160 rotas `GET`, com casa, pessoa e identificador de lixo — **nenhuma cai**; o filtro da 155 já as cobre, e a cobrança entrou na mesma suíte. **O sucesso que não acontece:** toda rota de escrita chamada pela coordenação da **Casa 04**, primeiro com identificador inventado, depois com a casa e a pessoa da Casa 03 no corpo, e por fim **sobre registros REAIS da Casa 03**, buscados no banco pelo catálogo (`house_id`, `person_id`, ou a ATA, a ocorrência, a receita a que pertencem). As duas primeiras voltaram quase limpas. **A terceira achou seis defeitos.** **(1) A tomada de conta:** `app_reset_staff_password` conferia o CARGO (a coordenação administra educador) e esquecia a CASA — a coordenação da Casa 04 recebia a senha de um educador que trabalha só na Casa 03, e entrava no sistema como ele. O convite (`app_issue_invite`) tinha a mesma falta: o link vai para o e-mail da pessoa, mas a senha dela é embaralhada no ato — tirava do sistema, no meio do plantão, quem não é da equipe de quem pediu. As outras três funções da equipe conferiam a casa desde a 0460. **(2) Quem SAIU de uma casa era administrável por qualquer coordenação**, e aparecia na lista da equipe de todas: "sem vínculo atual" era tratado como cargo institucional. *Foi a segunda rodada da sondagem que achou isto — depois do primeiro conserto, a conta de teste de outra suíte, com o vínculo encerrado, ainda aceitava tudo.* Hoje `app_casa_da_conta_no_alcance` responde pela casa atual ou, sem ela, pela ÚLTIMA, e as cinco funções e a lista usam a mesma resposta. **(3) A ciência no episódio da ATA** conferia só `user_id = eu`: a coordenação da Casa 04 gravava ciência COM COMENTÁRIO num episódio da Casa 03 que ela nem lê — a chave estrangeira não confere alcance, a lição da 146 outra vez. **(4) O acompanhante da internação** era trocado por quem é de outra casa: a política olhava só o cargo, e o serviço fechava o período de quem acompanhava sem ler a internação. **(5) O aviso marcado como ciente** sem existir respondia ok e **gravava a linha de auditoria de uma ciência que não houve.** **(6) O histórico do cofre** de uma criança de fora voltava vazio — *"ninguém abriu o cofre"* —, em vez de *"não é seu"* (regra 12). **A medição das políticas pelo catálogo** confirmou o quadro: das políticas de escrita sem nenhuma função de alcance, as de subconsulta já são recortadas pelo RLS da tabela consultada, as `false` só abrem por função, e as de "só a sua linha" (a própria conta, o próprio aviso) não cruzam casa; sobraram as duas acima e cinco cujos serviços leem o registro sob o RLS antes de gravar. **E um tropeço meu no caminho, que é a primeira lição da lista:** copiei `app_staff_list` da 0460, e a vigente era a da 0990, com uma coluna a mais — a migração quebrou no *"cannot change return type"*, e a cópia teria apagado a cor da linha. **E o conferidor de partições pegou outro:** as duas políticas estavam na migração da `identity`; foram para a `nursing` (1571) e a `shifts` (1572). A redefinição de senha passou a gravar a casa na auditoria (antes nascia sem). **Medido nos dois sentidos:** sem os consertos, as duas suítes reprovam dez testes; com eles, passam, e a terceira rodada da sondagem de alcance voltou limpa. |
| 157 | **A ATA passou a ser das oito às oito — e a regra passou a morar num lugar só.** A Fundação decidiu em 25/09: **diurna 08:00–20:00; noturna 20:01–07:59 do dia seguinte, da data em que começou** (a madrugada do dia 12 é da Noturna 11). Até aqui o sistema usava a hipótese 7h–19h (pendência #4), e ela estava **escrita à mão em seis funções do banco, de três partições** — a passagem que lê as doses e os retornos do turno, a ATA anterior que a próxima equipe lê, a escala que responde quem atende um compromisso, a dose confirmada e a dose "se necessário" —, **no servidor** (`tempo.ts`, numa variável de ambiente), **na tela da passagem** (`h >= 7 && h < 19`), **na escala** e **duas vezes no servidor de mentira**. Agora ela mora em `app_turno_de` e `app_janela_do_turno` (identity/1573), com os dois horários em `app_inicio_do_diurno()` e `app_inicio_do_noturno()`; as seis funções perguntam a elas (1574–1576, cada partição na sua); o servidor e a tela têm o espelho (`tempo.ts`, `frontend/src/turno.ts`). **A fronteira é por minuto**, como a pessoa lê o relógio: 20:00 (e 20:00:59) ainda é diurno, 20:01 já é noturno. **A variável de ambiente saiu** — uma variável é o jeito certo de o servidor discordar do banco em silêncio. **O que prova:** `a-ata-das-oito-as-oito.e2e.spec.ts` classifica onze instantes da fronteira (07:59, 08:00, 19:59, 20:00, 20:00:59, 20:01, 23:59, 00:00, 06:00, 07:59 e 08:00 do dia seguinte) no banco E no servidor, confere que a tela tem os mesmos dois horários, mede as janelas da Diurna e da Noturna de 11/09, e **pergunta ao CATÁLOGO** se alguma função ainda tem 07h/19h escrito. **Foi essa pergunta que achou a sexta função** — a dose "se necessário" (1390) —, que a minha varredura nos arquivos não viu porque olhava uma lista montada por mim. **E o ensaio de uso, rodado à 00h11, achou mais duas no servidor de mentira** — a janela do retorno da criança e a semente dele, escritas como `em(0, 7)` e `hora < 7`, sem a hora entre aspas que a busca procurava. Com elas, o protótipo mostrava na ATA o remédio pela regra nova e o retorno pela velha, e as duas consultas discordavam; às 22h, quando o ensaio tinha passado, os dois caminhos concordavam por coincidência. *Medido nos dois sentidos:* com as telas de antes da 157, na mesma hora, o ensaio só acusava a frase da escala. E o registro feito **sem internet** às 02:00 e sincronizado depois cai na Noturna do dia anterior: o instante que conta é o do aparelho, e a suíte lê a execução de volta pelo `app_turno_de`. |
| 158 | **A criança de outra casa deixou de parecer uma criança sem nada.** A medição que a 156 deixou: toda leitura `GET` do servidor, pedida pela coordenação da **Casa 04**, com os registros REAIS da Casa 03 no lugar de cada `:id`, buscados no banco que a suíte deixa povoado. **O critério foi o conteúdo:** resposta com o nome de uma criança ou de alguém da equipe da Casa 03 é vazamento. **Não vazou nada.** As duas respostas com nome da Casa 03 eram de crianças TRANSFERIDAS para a Casa 04 — a continuidade do cuidado manda a casa que recebe enxergar (§7.2) —, e a criança que ficou só na Casa 03 não aparece. **Mas medido pelo STATUS, 24 leituras respondiam 200 VAZIO a quem é de fora** — e o vazio dizia coisas falsas: **o dossiê listava cada documento exigido como FALTANDO** (*"esta criança não tem certidão"*), a saúde respondia *"nenhum atendimento"*, contatos, memórias, idas com a família e o rastro de relatório vinham vazios. É a regra 12. **O conserto é um só, na porta:** `@RegistroDaRota('personId', 'person')` declara de que tabela é o parâmetro, e o guarda `RegistroNoAlcance` (kernel) pergunta ao banco, COM a identidade de quem pede, se ele enxerga a linha — a pergunta é o próprio RLS, e nenhuma regra de alcance nova foi escrita. Está em **39 leituras**: as 24 medidas e toda leitura de criança (`:personId` em qualquer partição e `/people/:id`), com cobrança estática que nega por padrão a rota nova sem a declaração. **O rastro de relatório foi a exceção de ordem:** lá a conferência mora no serviço, DEPOIS da recusa por cargo, para o educador continuar recebendo *"quem lê o rastro é a coordenação ou a gestão"* e não um "não encontrado" que não ensina nada. **Dois testes antigos cobravam o 200 vazio** e foram reescritos com a razão: o da saúde, e o do rastro — cujo comentário dizia que distinguir "nada aconteceu" de "não é sua casa" exigiria olhar por fora do RLS; não exige, porque a pergunta é feita com a identidade de quem lê. **E três ataques que o pedido da Fundação nomeia, agora com teste:** a sessão antiga de quem foi desligado é recusada na próxima chamada (já era, sem teste); o educador não troca o próprio cargo pela API (já era); e o educador da Casa 03 digitando na URL o id de uma criança da Casa 04 recebe 404. |
| 159 | **Cada casa define o horário dos seus turnos.** Pedido da Fundação em 26/09: *"deixe o coordenador, educador líder, equipe técnica mudar de qual horário a qual horário fica o turno da manhã e da noite […] quando alterarem, o dia seguinte já sai no novo horário […] fica mais fácil para organizar na realidade de cada casa."* A 157 tinha posto a regra num lugar só; esta fase fez esse lugar perguntar PELA CASA. **A casa diz o diurno** (*"o diurno vai das 07:00 às 19:00"*) **e o noturno é o resto do dia** — começa um minuto depois e vai até o minuto antes do diurno seguinte. Dizer só o diurno é o que impede buraco e sobreposição. **Vale a partir do dia seguinte, nunca antes:** a ATA que passou guarda a janela que tinha, e a noite de HOJE termina no início do diurno NOVO de amanhã — se a casa antecipa de 08:00 para 07:00, a noturna de hoje acaba às 06:59 e nenhum minuto fica sem turno. **Quem muda:** coordenação, Líder Diurno e equipe técnica DA CASA, os três que o pedido nomeou e os mesmos que montam a escala; a coordenação de outra casa recebe 404. **Nada se apaga:** `house_shift_hours` só cresce, com autor, data e motivo opcional, e cada mudança vai à auditoria da casa; duas mudanças no mesmo dia, vale a mais recente. Casa sem configuração fica no padrão de 25/09. **As versões da regra sem casa SAÍRAM** — uma função que esquecesse a casa responderia o padrão calado —, e a suíte da 157 ganhou a pergunta ao catálogo que conta os argumentos de cada chamada. **Onde aparece:** um cartão *"Horário dos turnos"* na Escala, que mostra o horário de hoje, avisa *"a partir de DD/MM"* quando amanhã muda, **mostra como o noturno fica ANTES de gravar**, e lista quem mudou; a passagem abre o plantão pelo turno de agora DA CASA; a data do plantão noturno de madrugada sai do banco pelo horário da casa. A ATA Geral, que é das oito casas, fica no padrão. **Provado:** `o-horario-de-cada-casa.e2e.spec.ts` — padrão sem configuração, os três cargos e a recusa aos outros, hoje intacto e amanhã novo, a noite de hoje emendando no diurno novo, a outra casa intocada, histórico e auditoria; e o ensaio de uso faz a coordenação mudar o horário pela tela. |
...
153 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              106 suítes (e2e contra PostgreSQL real + estáticas)
...
│   └── ensaio*.mjs        os sete ensaios de navegador
...
### A medição que substituiu a pergunta "o que falta?" (fase 146)

*Com o §9 vazio dos dois lados, a pergunta deixou de ser "o que falta?" e passou a
ser **"o que nunca foi exercitado?"** — e essa tem resposta medível.*

**Como se mede, e por que a conta óbvia não serve.** Contar linhas depois da suíte
não distingue *"ninguém escreveu"* de *"a suíte limpou"*, e as suítes desta casa
limpam, porque o banco é um só. Quem separa os dois é o
`pg_stat_user_tables.n_tup_ins`, que conta as inserções da rodada **inclusive das
linhas apagadas depois**. Virou `scripts/superficies-sem-teste.mjs`, e ele roda
depois do `npm test`, na mesma rodada.

**A guarda dele é a parte mais importante.** O `globalSetup` do Jest recria o
schema a cada rodada e zera o contador — então rodá-lo depois de UMA suíte devolve
oitenta e três candidatas, todas falsas. Foi o que aconteceu na primeira vez. Ele
confere a proporção de tabelas tocadas e **se recusa a concluir** abaixo dela:
medidor que mente quando mal usado é pior do que medidor nenhum, porque a saída
dele tem a mesma cara nos dois casos.

**A última medição:** quatro tabelas vazias por decisão escrita — `work_schedule`
(MORTA), `medication_authorization` (dormente desde 08/09) e o
`medication_protocol` com o seu histórico, cujas rotas de escrita saíram naquela
decisão — e **nenhuma candidata**. Fora essas quatro, toda tabela do sistema é
exercitada por algum teste.

**Não é teste, e é a mesma decisão do `varredura-de-pontas.mjs`:** o resultado
precisa de julgamento, e um teste que reprovasse pelas quatro de decisão escrita
seria desligado na primeira pressa.

### As catorze rotas que continuam sem teste (medido na fase 147)

*O `scripts/rotas-sem-teste.mjs` mede, das 342 rotas do servidor, quais nenhum
teste chamou. A fase 147 fechou a família do vocabulário e os dois atos mais
sensíveis; estas catorze continuam abertas, e a lista está aqui para a próxima
sessão não ter de medir de novo.*

**Nenhuma delas trava o piloto** — são leituras e folhas —, e é por isso que a
fase 147 parou aqui em vez de escrever catorze testes de uma vez. A ordem abaixo é
a que eu escolheria:

1. ~~**`GET /audit/report/:id`**~~ ✅ **feita na fase 149, e ela achou o maior
   defeito desta sessão.** *E duas frases que eu escrevi aqui na 147 estavam
   erradas, as duas:* esta rota **não** é lida pela equipe técnica nem pelo Líder
   Diurno — quem a lê é a coordenação na própria casa, a gestão geral e a conta
   técnica (`LEEM` no serviço e a policy `audit_select`, os mesmos três) —, e ela
   **não** responde *"o que a Joana fez na terça"*: o controlador diz por extenso
   que a rota `/audit/actor/:id` NÃO existe, porque *"a mesma tabela que responde
   quem abriu o dossiê da Alice responderia tudo o que a Joana fez ontem, e a
   segunda pergunta é vigilância da equipe com outro nome"*. Entra-se pela CRIANÇA
   ou pelo REGISTRO, e por mais nada. **O defeito estava do outro lado da mesma
   linha da policy: 79 das 152 chamadas de `audit.log` gravavam sem casa, e linha
   sem casa é linha que a coordenação da casa não lê.** O relato está na 149.
2. ~~**`GET /medications/prn`**~~ ✅ **feita na fase 152, e ela tinha defeito** — o
   nome de quem deu a dose voltava NULO para a Enfermagem e o Líder Diurno, e o
   mesmo defeito estava no cargo de quem escreve cada linha da ATA. O relato está
   na 152.
3. ~~**`PATCH /shifts/general-night-line/:data/house/:houseId`**~~ ✅ **feita na
   fase 148, e ela tinha defeito.** *A frase que eu escrevi aqui ontem estava ao
   contrário:* não é "a escrita original sem teste" — esta É a correção da linha
   pela DATA (1440), criada para o identificador da folha das oito casas não sair
   do servidor, e a suíte da fase 138 exercita a OUTRA, a que leva o id. Ela nunca
   tinha sido chamada, e devolvia **500** para uma data que não é data.
4. ~~**`GET /timeline/all`** e **`GET /transfers/pending`**~~ ✅ **fase 153.** A
   primeira estava certa por dentro e ganhou teste; a leitura dela é que achou o
   defeito maior da fase — **dez `@Query('date')` sem conferência**, e quatro
   delas devolvendo 500 na chamada, no plantão, nas atividades e na grade do
   remédio. A segunda **saiu**: ninguém a chamava, e a frase que a justificava
   (*"contagem, para aviso"*) era falsa — ela devolvia a lista.
5. ~~**As cinco EXPORTAÇÕES** e as **três FOLHAS**~~ ✅ **fase 154, e elas
   tinham defeito.** A ficha de saúde e a trajetória gravavam a auditoria sem
   casa; a folha do remédio da ida gravava a casa que a tela mandava; sete
   exportações caíam com 500 por casa ou data inválida no corpo; e as folhas da
   cozinha diziam *"0 crianças com restrição"* a quem não alcança a casa. *A
   frase que estava aqui — "o caminho de exportar é testado noutras rotas, e o
   que falta é a garantia por rota" — era o raciocínio que deixa defeito passar:
   o caminho comum estava certo, e o erro morava no que cada rota passava a ele.*
6. ~~**`GET /medications/authorizations`**~~ ✅ **fase 154**, com a cobrança
   mínima: a tela da saúde ainda a lê, e ela responde. O comportamento da
   autorização nominal, dormente desde 08/09 (§8.6), não ganhou teste — testar
   o que a Fundação aposentou é guardar o passado.

**Com a 154, o `rotas-sem-teste.mjs` não aponta nenhuma candidata forte.**
...
**O que as fases 141, 142 e 143 consertaram está no §2.** As três coisas que
faltavam foram feitas; o que sobra são as DUAS DECISÕES, no fim desta seção.

1. ~~**A portaria não tem DIA nem HORÁRIO de visita.**~~ ✅ **feito na fase 142.**
   Cada visitante autorizado tem os dias e a faixa de horário dele, e eles saem
   impressos na coluna *"Quando pode vir"* da folha da guarita. Quem ainda não
   combinou aparece com a instrução escrita, e não com um traço.
2. ~~**Retirar a autorização de visita não pede motivo.**~~ ✅ **feito na fase
   142.** O motivo é obrigatório, com dez caracteres, e vive em
   `contact_visit_change` — append-only, lido pelos três que respondem por quem
   entra, e fora do log, porque a frase conta algo sobre uma família.
3. ~~**O educador não alcança o Arquivo de ATAS.**~~ ✅ **feito na fase 143.** O
   dado já era dele — `ata_select` responde por casa e `note_select` já excluía a
   linha restrita —; o que o barrava era a lista de cargos na porta. A ATA Geral
   Noturna continua fora, porque a **§10.2** é da Fundação, e a tela diz isso em
   vez de só omitir.

**E as duas decisões que eram dela chegaram em 22/09 e viraram a fase 144:**

* ~~**Retirar alguém de um plantão FUTURO pede motivo?**~~ ✅ **respondido: sempre
  exigir.** O argumento da 1310 — *"mudar o futuro é organização"* — continua
  correto sobre o histórico e não era o único em jogo: quem pergunta *"por que eu
  fui tirado do plantão de sábado?"* pergunta na segunda, sobre um plantão que era
  futuro quando a mudança foi feita. Vale para a substituição também, e a frase da
  recusa muda com o tempo do plantão, porque o motivo é outro.
* ~~**O Líder Diurno lê ocorrência protegida?**~~ ✅ **respondido: incluir.** A
  assimetria era real — o relato restrito o incluía desde a 0730 e o bloco
  protegido não —, e eu não a desfiz sozinho porque ocorrência protegida é
  material mais pesado que relato. **E a leitura passou a deixar linha, para todos
  os cargos:** ampliar quem lê o material mais pesado do sistema sem rastro nenhum
  não se defende numa audiência, e a casa já tinha o precedente
  (`person.judicial_view`). Não é a porta com finalidade escrita, que ela recusou
  para este caso.

**E a §10.2 também foi respondida, em 22/09** — por um lado que eu não tinha
previsto. A 1510 deixou a ATA Geral fora de quem não a lia justamente para não
responder de carona uma pergunta da Fundação; ela respondeu: *"todos leem a ata
coletiva, seja manhã ou noite"*. A **fase 145** abriu a linha desta casa na Geral
para todo cargo que cuida dos acolhidos, e criou o que a segunda metade da frase
dela pedia — *"a pessoa pode solicitar ler alguma coisa, e cabe à equipe deixar ou
não"*.

**O §9 está vazio do lado da Fundação e do lado do código.** O que resta é
aplicar o roteiro com a Casa 03, que não se faz daqui.
...
a medida que falta** — sete ensaios de navegador dizem que a porta existe, e
...
| 6 | ~~**O horário oficial do Líder Noturno Geral**~~ ✅ **Respondida em 25/09 (fase 157)** | *ATA diurna 08:00–20:00; noturna 20:01–07:59, da data em que começou.* A variável `NIGHT_SHIFT_END_HOUR` saiu: a regra mora no banco (`app_turno_de`, `app_janela_do_turno`) e o servidor e a tela a espelham, com teste que os obriga a concordar |
...
Constrói, cria um banco virgem, aplica as 153 migrações **pelo binário
