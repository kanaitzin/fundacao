...
| **141 migrações** | `.sql` dentro das partições |
| **116 tabelas** | `CREATE TABLE` nas migrações |
| **98 suítes** | `backend/test/*.spec.ts` |
| **943 testes** | `it(` / `test(` nas suítes |
...
141 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              98 suítes (e2e contra PostgreSQL real + estáticas)
...
2. **`GET /medications/prn`** — a lista do remédio "se necessário" (fase 132), que
   é o que o educador dá às 2h da manhã. A rota de ESCREVER tem teste; a de LER,
   não.
...
Constrói, cria um banco virgem, aplica as 141 migrações **pelo binário
