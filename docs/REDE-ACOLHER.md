...
| **145 migrações** | `.sql` dentro das partições |
| **116 tabelas** | `CREATE TABLE` nas migrações |
| **103 suítes** | `backend/test/*.spec.ts` |
| **984 testes** | `it(` / `test(` nas suítes |
...
145 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              103 suítes (e2e contra PostgreSQL real + estáticas)
...
Constrói, cria um banco virgem, aplica as 145 migrações **pelo binário
