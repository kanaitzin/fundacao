...
| **95 suítes** | `backend/test/*.spec.ts` |
| **920 testes** | `it(` / `test(` nas suítes |
...
│   ├── test/              95 suítes (e2e contra PostgreSQL real + estáticas)
...
1. **`GET /audit/report/:id`** — a leitura da auditoria, que a fase 117 abriu para
   a coordenação, a equipe técnica e o Líder Diurno. É a rota que responde *"o que
   a Joana fez na terça"*, e ela é a única da lista que fala sobre uma PESSOA da
   equipe: um teste aqui guarda a forma da resposta, para o total não voltar de
   carona (a fase 117 tem um teste da forma, mas não desta rota).
