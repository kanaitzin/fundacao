...
| **125 migrações** | `.sql` dentro das partições |
| **112 tabelas** | `CREATE TABLE` nas migrações |
| **80 suítes** | `backend/test/*.spec.ts` |
| **795 testes** | `it(` / `test(` nas suítes |
...
125 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              80 suítes (e2e contra PostgreSQL real + estáticas)
...
**5. Cinco começos que não ficaram ligados em nada — e dois deles já ficaram.**

*A `work_schedule` morreu de verdade nas fases 129 e 131; o "se necessário" ficou ligado na
132. **Sobram TRÊS**, e a linha de cada uma diz o que ainda falta.*
...
Constrói, cria um banco virgem, aplica as 125 migrações **pelo binário
