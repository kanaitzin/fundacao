...
| **142 migrações** | `.sql` dentro das partições |
| **116 tabelas** | `CREATE TABLE` nas migrações |
| **102 suítes** | `backend/test/*.spec.ts` |
| **974 testes** | `it(` / `test(` nas suítes |
...
142 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              102 suítes (e2e contra PostgreSQL real + estáticas)
...
Constrói, cria um banco virgem, aplica as 142 migrações **pelo binário
