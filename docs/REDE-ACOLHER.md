...
| **94 suítes** | `backend/test/*.spec.ts` |
| **915 testes** | `it(` / `test(` nas suítes |
...
│   ├── test/              94 suítes (e2e contra PostgreSQL real + estáticas)
...
2. **`GET /medications/prn`** — a lista do remédio "se necessário" (fase 132), que
   ...
   não.
3. **`PATCH /shifts/general-night-line/:id/house/:id`** — a linha da casa na ATA
   Geral, escrita pelo Líder Noturno. A correção dela (fase 138) tem suíte; a
   escrita original, não.
