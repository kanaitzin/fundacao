...
| **100 suítes** | `backend/test/*.spec.ts` |
| **956 testes** | `it(` / `test(` nas suítes |
...
│   ├── test/              100 suítes (e2e contra PostgreSQL real + estáticas)
...
5. **As cinco EXPORTAÇÕES** (`alignments/export`, `escala/export`,
   `medications/export`, `to-take/export`, `nursing/history/:id/export`) e as
   **três FOLHAS** (`to-take/folha`, `kitchen-requests/folha/restricoes`,
   `folha/cestas`). Todas geram documento e registram finalidade; o caminho de
   exportar é testado noutras rotas, e o que falta é a garantia por rota.
6. **`GET /medications/authorizations`** — leitura da autorização nominal, que é
   dormente por decisão escrita (§8.6). É a única da lista que talvez não precise
   de teste: testar o que a Fundação decidiu aposentar é guardar o passado.
