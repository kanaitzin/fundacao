...
|---|---|
| `der.md` | as 116 tabelas com o que cada coluna guarda. É referência de dado, não narrativa, e o `documentacao.spec.ts` cobra que toda tabela apareça lá |
...
| **149 migrações** | `.sql` dentro das partições |
| **116 tabelas** | `CREATE TABLE` nas migrações |
| **105 suítes** | `backend/test/*.spec.ts` |
| **999 testes** | `it(` / `test(` nas suítes |
...
149 migrações em ordem e aplica os seeds (`seed.ts`, `seed-fase2.ts`, `seed-fase4.ts`).
...
│   ├── test/              105 suítes (e2e contra PostgreSQL real + estáticas)
...
Constrói, cria um banco virgem, aplica as 149 migrações **pelo binário
